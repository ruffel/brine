...
COMPOSE=("${ROOT_DIR}/scripts/compose.sh" -f "${COMPOSE_FILE}")
...
while (( SECONDS < end )); do
  if "${COMPOSE[@]}" exec -T salt-master salt-key --list=accepted --out=json >/tmp/brine-salt-keys.json 2>/dev/null; then
...
PY
    ...
    if [[ "${accepted_count}" -ge "${EXPECTED_MINIONS}" ]]; then
      if "${COMPOSE[@]}" exec -T salt-master salt '*' test.ping --out=json --static >/tmp/brine-test-ping.json 2>/dev/null; then
...
"${COMPOSE[@]}" logs --tail=80 salt-master >&2 || true
printf '\nRecent minion logs:\n' >&2
minion_names=()
for i in $(seq 1 "${EXPECTED_MINIONS}"); do
  minion_names+=("minion-${i}")
done
"${COMPOSE[@]}" logs --tail=80 "${minion_names[@]}" >&2 || true
exit 1
